...
func fakeHandler(val string) HandlerFunc {
	return func(*http.Request, Params) Response {
		fakeHandlerValue = val
		return ResponseText(200, "test")
	}
}
...
func checkRequests(t *testing.T, tree *node, requests testRequests) {
	for _, request := range requests {
		...
		default:
			//handler(nil, nil, nil)
			handler.HandleRequest(nil, nil, nil)
...
func TestTreeInvalidNodeType(t *testing.T) {
	...
	r1 := &Route{
		Path:    "/:page",
		Handler: fakeHandler("/:page"),
	}
	tree.addRoute("/:page", r1)
...
func TestTreeWildcardConflictEx(t *testing.T) {
	...
	for i := range conflicts {
		...
		recv := catchPanic(func() {
			r := &Route{
				Path:    conflict.route,
				Handler: fakeHandler(conflict.route),
			}
			tree.addRoute(conflict.route, r)
		})
