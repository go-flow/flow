...
import (
	...
	testdata "github.com/go-flow/flow/testdata/protoexample"
)
...
func TestContextHandlerName(t *testing.T) {
	...
	assert.Regexp(t, "^(.*/vendor/)?github.com/go-flow/flow.handlerNameTest$", c.HandlerName())
}
...
func TestContextRenderHTML2(t *testing.T) {
	...
	app.router.Handle("GET", "/", func(_ *Context) {})
	assert.Len(t, app.router.trees, 1)
...
func TestContextRenderFile(t *testing.T) {
	...
	assert.Contains(t, w.Body.String(), "func Default() *App {")
	assert.Equal(t, "text/plain; charset=utf-8", w.Header().Get("Content-Type"))
}
